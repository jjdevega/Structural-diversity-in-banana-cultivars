...
K6<- read.csv("K6_ordered_final.csv",header=TRUE)
...
### plot unknown
K6<- read.csv("K6_new_unclustered34.csv",header=TRUE)
...
pdf(file = "new_plot_unclustered_K6.pdf",width=10,height=5)
cog4
dev.off()
...
### order K4  
K6<- read.csv("K6_order_plot_gp.csv",header=TRUE)

order <- read.csv("../../plot_order_groups.csv",header = TRUE)

# output K6 in same order as 
all <-left_join(order,K6,by = "Taxa")
write.csv(all,"K6_ordered.csv")



### reorder K4 and K5

K4<- read.csv("K4_ordered_plot.csv",header=TRUE)
all <-left_join(K6,K4,by = "nssp")
write.csv(all,"K4_order_plot.csv")

K5<- read.csv("K5_ordered_plot.csv",header=TRUE)
all <-left_join(K6,K5,by = "nssp")
write.csv(all,"K5_order_plot.csv")

## remove columns, add Q labels and read back in
K4<- read.csv("K4_order_plot.csv",header=TRUE)
K5<- read.csv("K5_order_plot.csv",header=TRUE)
K6<- read.csv("K6_order_plot.csv",header=TRUE)
...
cog4 <- cog3 + scale_fill_manual(values=pcaPalette)
cog5 <- cog4 +     facet_wrap(~pops,nrow=3) 

pdf(file = "plot_aav4_K4_5_6.pdf",width=18,height=8)
cog5
dev.off()
